...
function ProposalContent ({ proposal }: any) {
  const voteOpacity = proposal.status === STATUSES.pending ? '1' : '0.4';
  const vetoOpacity = proposal.status === STATUSES.accepted ? '1' : '0.4';

  const approvalContracts = proposal.contract === CONTRACTS_NAMES.addressVoting ||
    proposal.contract === CONTRACTS_NAMES.upgradeVoting;

  const contractsWithoutVeto = proposal.contract === CONTRACTS_NAMES.validatorsSlashingVoting ||
