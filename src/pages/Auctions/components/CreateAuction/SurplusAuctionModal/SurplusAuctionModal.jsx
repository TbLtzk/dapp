...
function SurplusAuctionModal ({ modalOpen, onHide }) {
  ...
  const { values, stepIndex, goNext, goBack, confirm, reset } = useMultiStepForm({
    ...
    onConfirm: (values) => {
      dispatch(createAuction({
        ...values,
        contract: AUCTIONS_TYPES.systemSurplus
      }));
    },
  });
  ...
  return (
    <LocalStateContext.Provider value={{ values, goNext, goBack, confirm }}>
      <MultiStepModal
        ...
        onHide={handleHide}
      >
        <AuctionStep surplusLot={surplusLot} />
        <ConfirmStep surplusLot={surplusLot} />
      </MultiStepModal>
 ...
 * }}
 */
export const useSurplusAuction = () => useContext(LocalStateContext);

export default SurplusAuctionModal;
