...
interface GovPubProposedExclusionListRpc {
...
export async function fetchGovPubProposedExclusionList (
  provider: GovPubRpcProvider,
): Promise<GovPubExclusionList | null> {
  const result: GovPubProposedExclusionListRpc | null = await provider.send(
    'govPub_proposedExclusionList',
    [],
  );
