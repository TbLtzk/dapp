...
import { getQVaultAmount, getRootNodeAmount, getValidatorAmount, setDepositLockedAmount } from 'store/actions/action-creaters/locked-amount';
...
function TimeLocks() {
  ...
  const setDeposit  = (data, contractMethod) => {
    dispatch(setDepositLockedAmount({data, contractMethod, userAddress}))
  }
  ...
  return (
    <PageWrap headerTitle="Time Locks">
      ...
      <InfoWrap>
        <BalancePage timeLockBalance={qVaultMin} balance={userQVBalance} setDeposit={(data) => setDeposit(data, 'qVault')} contract='qVault' title="Q Vault account balance" lockAmountData={qVaultLockedAmount === 0 ? [] : qVaultLockedAmount.lockedQVaultAmounts} />
        <BalancePage timeLockBalance={rootNodeMin} balance={'ROOT'} setDeposit={(data) => setDeposit(data, 'root')} contract='root' title="Root stake balance" lockAmountData={rootNodeLockedAmount === 0 ? [] : rootNodeLockedAmount.lockedRootNodeAmounts} />
        <BalancePage timeLockBalance={validatorMin} balance={'VALIDATOR'} setDeposit={(data) => setDeposit(data, 'validator')} contract='validator' title="Validator stake balance" lockAmountData={validatorLockedAmount === 0 ? [] : validatorLockedAmount.lockedValidatorAmounts} />
